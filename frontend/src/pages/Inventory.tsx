...
export default function Inventory({ isAuthenticated }: InventoryProps) {
  ...
  const handleSaveQuantity = async (item: InventoryItem) => {
    ...
    try {
      ...
      setAlertMessage(err.message || 'Failed to update quantity')
      setShowAlertDialog(true)
    } finally {
      setSavingEdit(false)
    }
                        ...
                        <div className="bg-slate-800 rounded-lg border border-slate-700 p-5 hover:border-emerald-500 transition-colors">
                          ...
                          <div className="mt-4 pt-4 border-t border-slate-700 space-y-2">
                            {editingItemId === item.id ? (
                              ...
                              >
                                <div className="flex gap-2">
                                  <input
                                    ...
                                    min="0"
                                    step="0.1"
                                    className="flex-1 px-3 py-1.5 text-sm bg-slate-900 border border-emerald-500 rounded text-white focus:outline-none"
                                    autoFocus
                                  />
                                  <select
                                    value={editUnit}
                                    onChange={(e) => setEditUnit(e.target.value)}
                                    className="px-3 py-1.5 text-sm bg-slate-900 border border-slate-700 rounded text-white focus:outline-none focus:border-emerald-500"
                                  >
                                    {[...UNIT_OPTIONS.volume, ...UNIT_OPTIONS.mass, ...UNIT_OPTIONS.count].map(u => (
                                ...
                                <div className="flex gap-2">
                                  <button
                                    ...
                                    disabled={savingEdit || !editQuantity}
                                    className="flex-1 px-3 py-1.5 text-sm bg-emerald-500 hover:bg-emerald-600 disabled:bg-slate-700 text-white rounded font-medium transition-colors"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="flex-1 px-3 py-1.5 text-sm bg-slate-700 hover:bg-slate-600 text-white rounded font-medium transition-colors"
                                  >
                                    Cancel
                                  </button>
                                </div>
                              ...
                              <div
                                className="flex justify-between items-center cursor-pointer rounded-md px-2 py-1.5 hover:bg-slate-700/50 border border-transparent hover:border-slate-600 transition-all group/qty"
                                onClick={(e) => {
                                  e.preventDefault()
                                  e.stopPropagation()
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs text-slate-600 group-hover/qty:text-slate-400 transition-colors">
                                    ✏️
                                  </span>
                                </div>
                              </div>
